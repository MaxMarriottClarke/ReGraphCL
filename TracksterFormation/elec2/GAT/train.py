...
class Net(nn.Module):
    def __init__(self, hidden_dim=64, k_value=24, contrastive_dim=8, heads = 1):
        ...
        self.conv1 = GATConv(
            in_channels=self.hidden_dim,  # Input channels
            out_channels=self.hidden_dim,  # Output channels to match the hidden dimension
            heads=self.heads ,  # Number of attention heads (adjust if needed)
            concat=False  # Keep the output dimension the same as `out_channels`
        )
    ...
    def forward(self, x_lc, batch_lc, edge_index):
        x_lc_enc = self.lc_encode(x_lc)
        feats1 = self.conv1(x_lc_enc, edge_index)
        feats2 = self.conv1(feats1, edge_index) 
        feats3 = self.conv1(feats2, edge_index) 
        out = self.output(feats3)
        return out, batch_lc
        
def train(loader, model, optimizer, device, temperature, k = 16):
    ...
    for data in tqdm.tqdm(loader):
        data = data.to(device)  # Send data to device (GPU/CPU)
        optimizer.zero_grad()  # Reset gradients
        
        edge_index = knn_graph(data.x_lc[:, :3], k=k, batch=data.x_lc_batch)
        
        out = model(data.x_lc, data.x_lc_batch, edge_index)  # Forward pass
        
        # Calculate loss (implement your loss calculation here)
        # This part assumes you have implemented a `contrastive_loss` function
        values, counts = np.unique(data.x_lc_batch.detach().cpu().numpy(), return_counts=True)
        losses = []
        for e in range(len(counts)):
            dummy_tensor = torch.randn(200,200, device='cpu')  # Forces CPU usage
            dummy_result = torch.matmul(dummy_tensor, dummy_tensor)  # Matrix multiplication on CPU
...
@torch.no_grad()
def test(loader, model, device, temperature, k = 16):
    ...
    for data in tqdm.tqdm(loader):
        ...
        # Calculate loss (similar to train)
        values, counts = np.unique(data.x_lc_batch.detach().cpu().numpy(), return_counts=True)
        losses = []
        for e in range(len(counts)):
            dummy_tensor = torch.randn(200,200, device='cpu')  # Forces CPU usage
