...
class Net(nn.Module):
    def __init__(self, hidden_dim=64, k_value=24, contrastive_dim=8, heads = 4):
...
def train(loader, model, optimizer, device, temperature, k = 24):
    model.train()
    total_loss = 0
    ...
    for data in loader:
        ...
        out = model(data.x_lc, data.x_lc_batch, edge_index)
...
@torch.no_grad()
def test(loader, model, device, temperature, k = 24):
    model.eval()
    total_loss = 0
