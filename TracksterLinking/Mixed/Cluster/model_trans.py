...
# Assume GraphTransformerLayer is defined elsewhere.
# Example network using GraphTransformerLayer.
class Net(nn.Module):
    def __init__(self, hidden_dim=64, num_layers=4, dropout=0.3, contrastive_dim=8, num_heads=4):
        """
        Initializes a graph transformer network with dual heads:
         - One head produces contrastive embeddings.
         - The other head produces node-level split_logit for binary classification.

        The split_logit branch uses a skip connection from the early encoded features 
        to incorporate multi-scale information.
        """
        ...
        # Optional skip projection for early features.
        self.skip_proj = nn.Linear(hidden_dim, 64)
        ...
        # This head takes as input the concatenation of the shared features and a skip connection from the early encoder.
        self.split_head = nn.Sequential(
            nn.Linear(64 + 64, 32),  # combining shared and skip-projected features.
            nn.ELU(),
            nn.Dropout(p=dropout),
    ...
    def forward(self, x, edge_index, batch=None):
        ...
        # Save early features for a skip connection.
        skip_features = self.skip_proj(x_enc)  # (N, 64)
        ...
        shared_features = self.shared_out(x_enc)  # (N, 64)
        
        # Contrastive head.
        contrastive_out = self.contrastive_head(shared_features)  # (N, contrastive_dim)
        
        # Combine shared features with skip connection for split classification.
        combined_features = torch.cat([shared_features, skip_features], dim=-1)  # (N, 128)
        split_logit = self.split_head(combined_features)  # (N, 1)

        # Return both outputs along with the batch vector.
