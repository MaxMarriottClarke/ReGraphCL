...
class CCV1(Dataset):
    ...
    def fill_data(self, max_events):
        ...
        for path in tqdm(self.raw_paths):
            ...
            tracksters_path = find_highest_branch(path, 'ticlDumper/tracksters')
            associations_path = find_highest_branch(path, 'ticlDumper/associations')
            simtrack = find_highest_branch(path, 'ticlDumper/simtrackstersCP')
            ...
            for array in uproot.iterate(
                ...
                for vert_chunk in uproot.iterate(
                    f"{path}:{simtrack}",
                    ["barycenter_x", "vertices_indexes", "vertices_multiplicity"],
                ):
                    vert_array = vert_chunk["barycenter_x"]
                    GT_ind = vert_chunk["vertices_indexes"]
                    GT_mult = vert_chunk["vertices_multiplicity"]
