...
data_train = CCV1(ipath, max_events=5000)
data_val = CCV1(vpath, max_events=10000, inp = 'val')

# Initialize model with passed hyperparameters
model = Net(hidden_dim=args.hidden_dim, num_layers=args.num_layers, dropout=0.3, contrastive_dim=args.contrastive_dim, num_heads = args.num_heads).to(device)
...
output_dir = '/vols/cms/mm1221/hgcal/elec5New/Track/Hyperparam/results/Trans/'
result_path = (
    f'results_lr{args.lr}_bs{args.batch_size}_hd{args.hidden_dim}_nl{args.num_layers}_'
    f'nh{args.num_heads}_cd{args.contrastive_dim}_k{args.k_value}/'
)
...
result_filename = (
    f'results_lr{args.lr}_bs{args.batch_size}_hd{args.hidden_dim}_nl{args.num_layers}_'
    f'nh{args.num_heads}_cd{args.contrastive_dim}_k{args.k_value}.csv'
)
...
for epoch in range(args.epochs):
    ...
    train_loss = train_new(train_loader, model, optimizer, device, k_value=args.k_value, temp=0.1, alpha=alpha)
    val_loss = test_new(val_loader, model, device, k_value=args.k_value, temp=0.1)
...
hyperparam_results_file = "hyperparam_summary_Trans.csv"
