...
def train_new(train_loader, model, optimizer, device, k_value, temp, alpha):
    model.train()
    ...
    for data in tqdm(train_loader, desc="Training"):
        ...
        for count in counts:
            ...
            loss_event = contrastive_loss_curriculum(event_embeddings,
                                                     event_group_ids, temperature=temp, alpha=alpha)
...
@torch.no_grad()
def test_new(test_loader, model, device, k_value, temp):
    model.eval()
    total_loss = torch.zeros(1, device=device)
    for data in tqdm(test_loader, desc="Validation"):
        ...
        for count in counts:
            ...
            event_group_ids = assoc_tensor[start_idx:end_idx]
            loss_event = contrastive_loss_curriculum(event_embeddings,
                                                     event_group_ids, temperature=temp, alpha=1)
            loss_event_total += loss_event
            start_idx = end_idx
