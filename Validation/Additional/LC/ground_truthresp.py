...
class CCV3(Dataset):
    ...
    def fill_data(self,max_events):
        ...
        for fi,path in enumerate(tqdm.tqdm(self.raw_paths)): ## only one root file so is there a need for a for loop?
            ...
            #if fi > 2:
                #break
            cluster_path = find_highest_branch(path, 'ticlDumper/clusters')
            sim_path = find_highest_branch(path, 'ticlDumper/simtrackstersCP')
            trackster_path = find_highest_branch(path, 'ticlDumper/tracksters')
    ...
    def get(self, idx):
        ...
        x_counts = lc_x
        return Data(x=x, x_counts = x_counts, idx=flat_lc_id)
