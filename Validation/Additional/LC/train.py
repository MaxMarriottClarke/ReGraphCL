...
class CCV1(Dataset):
    ...
    def fill_data(self,max_events):
        ...
        for fi,path in enumerate(tqdm(self.raw_paths)):
            ...
            "vertices_energy", "vertices_multiplicity", "vertices_time", "vertices_indexes", "barycenter_x", "barycenter_y", "barycenter_z"], step_size=self.step_size):
                ...
                tmp_all_vertices_time = crosstree['cluster_time'].array(entry_start=crosscounter*self.step_size,entry_stop=(crosscounter+1)*self.step_size)
                tmp_all_vertices_type = crosstree['cluster_type'].array(entry_start=crosscounter*self.step_size,entry_stop=(crosscounter+1)*self.step_size)
                ...
                tmp_stsCP_vertices_indexes_repeats_only = tmp_stsCP_vertices_indexes[skim_mask_noh]
                tmp_stsCP_vertices_multiplicity_repeats_only = tmp_stsCP_vertices_multiplicity[skim_mask_noh]
                ...
                tmp_ts_vertices_x = tracktree['vertices_x'].array(entry_start=crosscounter*self.step_size,entry_stop=(crosscounter+1)*self.step_size)
                tmp_ts_vertices_y = tracktree['vertices_y'].array(entry_start=crosscounter*self.step_size,entry_stop=(crosscounter+1)*self.step_size)
                tmp_ts_vertices_z = tracktree['vertices_z'].array(entry_start=crosscounter*self.step_size,entry_stop=(crosscounter+1)*self.step_size)
                crosscounter += 1
                ...
                tmp_stsCP_vertices_indexes_repeats_only = tmp_stsCP_vertices_indexes_repeats_only[skim_mask]
                tmp_stsCP_vertices_multiplicity_repeats_only = tmp_stsCP_vertices_multiplicity_repeats_only[skim_mask]
                tmp_stsCP_barycenter_x = tmp_stsCP_barycenter_x[skim_mask]
                tmp_stsCP_barycenter_y = tmp_stsCP_barycenter_y[skim_mask]
                tmp_stsCP_barycenter_z = tmp_stsCP_barycenter_z[skim_mask]
                tmp_ts_vertices_x = tmp_ts_vertices_x[skim_mask]
                tmp_ts_vertices_y = tmp_ts_vertices_y[skim_mask]
                tmp_ts_vertices_z = tmp_ts_vertices_z[skim_mask]
                tmp_all_vertices_layer_id = tmp_all_vertices_layer_id[skim_mask]
                tmp_all_vertices_noh = tmp_all_vertices_noh[skim_mask]
                tmp_all_vertices_eta = tmp_all_vertices_eta[skim_mask]
                tmp_all_vertices_phi = tmp_all_vertices_phi[skim_mask]
                tmp_all_vertices_energy = tmp_all_vertices_energy[skim_mask]
                tmp_all_vertices_x = tmp_all_vertices_x[skim_mask]
                tmp_all_vertices_y = tmp_all_vertices_y[skim_mask]
                tmp_all_vertices_z = tmp_all_vertices_z[skim_mask]
                tmp_all_vertices_time = tmp_all_vertices_time[skim_mask]
                tmp_all_vertices_type = tmp_all_vertices_type[skim_mask]
                ...
                if counter == 0:
                    ...
                    self.stsCP_vertices_indexes_repeats_only = tmp_stsCP_vertices_indexes_repeats_only
                    self.stsCP_vertices_multiplicity_repeats_only = tmp_stsCP_vertices_multiplicity_repeats_only
                    ...
                    self.ts_vertices_x = tmp_ts_vertices_x
                    self.ts_vertices_y = tmp_ts_vertices_y
                    self.ts_vertices_z = tmp_ts_vertices_z
                    ...
                    self.all_vertices_noh = tmp_all_vertices_noh
                    self.all_vertices_eta = tmp_all_vertices_eta
                    self.all_vertices_phi = tmp_all_vertices_phi
                    ...
                    self.all_vertices_time = tmp_all_vertices_time
                    self.all_vertices_type = tmp_all_vertices_type
                    ...
                    self.stsCP_vertices_indexes_repeats_only = ak.concatenate((self.stsCP_vertices_indexes_repeats_only,tmp_stsCP_vertices_indexes_repeats_only))
                    self.stsCP_vertices_mutiplicity_repeats_only = ak.concatenate((self.stsCP_vertices_multiplicity_repeats_only,tmp_stsCP_vertices_multiplicity_repeats_only))
                    ...
                    self.ts_vertices_x = ak.concatenate((self.ts_vertices_x,tmp_ts_vertices_x))
                    self.ts_vertices_y = ak.concatenate((self.ts_vertices_y,tmp_ts_vertices_y))
                    self.ts_vertices_z = ak.concatenate((self.ts_vertices_z,tmp_ts_vertices_z))
                    ...
                    self.all_vertices_noh = ak.concatenate((self.all_vertices_noh,tmp_all_vertices_noh))
                    self.all_vertices_eta = ak.concatenate((self.all_vertices_eta,tmp_all_vertices_eta))
                    self.all_vertices_phi = ak.concatenate((self.all_vertices_phi,tmp_all_vertices_phi))
                    ...
                    self.all_vertices_time = ak.concatenate((self.all_vertices_time,tmp_all_vertices_time))
                    self.all_vertices_type = ak.concatenate((self.all_vertices_type,tmp_all_vertices_type))
    ...
    def get(self, idx):
        ...
        # Loop over particles to build positive and negative edges
        pos_edges = []
        neg_edges = []
        pos_lc_label = []
        neg_lc_label = []
        ...
        for cp in range(len(lc_x)):
            ...
            for lc in range(n_lc_cp):
                ...
                for other_cp in range(len(lc_x)):
                    if other_cp != cp:
                        for other_lc in range(len(lc_x[other_cp])):
                            neg_edges.append([offset + lc, sum(len(lc_x[i]) for i in range(other_cp)) + other_lc])
                            neg_lc_label.append(offset + lc) # Track current LC for negative edge
            offset += n_lc_cp
        ...
        x_neg_edge = torch.from_numpy(np.array(neg_edges).astype(np.int64))
        pos_lc_label = torch.tensor(pos_lc_label, dtype=torch.long)
        neg_lc_label = torch.tensor(neg_lc_label, dtype=torch.long)
                        
        x_counts = lc_x
        y = torch.from_numpy(np.array([0 for u in range(len(flat_lc_feats))])).float()
        return Data(
            x=x, x_counts=x_counts, x_pe=x_pos_edge, x_ne=x_neg_edge, lc_ne=neg_lc_label
        )
...
class Net(nn.Module):
    def __init__(self, hidden_dim=64, k_value=24, contrastive_dim=8):
        ...
        self.output = nn.Sequential(
            ...
            nn.Linear(32, 16),
            nn.ELU(),
            nn.Linear(16, self.contrastive_dim),
        )
