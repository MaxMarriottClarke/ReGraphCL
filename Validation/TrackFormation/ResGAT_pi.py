...
model = Net_GAT(128,3, dropout=0.3, contrastive_dim=512, num_heads=4)
checkpoint= torch.load('/vols/cms/mm1221/hgcal/Mixed/LC/Full/runs/GAT/hd128nl3cd512k16h4/epoch-99.pt',  map_location=torch.device('cpu'))
...
for i, data in enumerate(test_loader):
    edge_index = knn_graph(data.x[:, :3], k=16, batch=data.x_batch)
...
df_CL.to_csv('df_GAT_pi_res.csv', index=False)
df_TICL = calculate_all_event_scores(GT_ind_filt, energies, Track_ind, TrueEnergy, GT_mult_filt, num_events = len(recon_ind))

df_TICL.to_csv('df_Track_pi_res.csv', index=False)
