...
# Also, testpath should point to your test data directory.
testpath = '/vols/cms/mm1221/Data/mix/test/'

# Set the number of runs for each CP value to compute error bars
num_runs = 4  # Adjust the number of runs if needed
cp_values = list(range(1, 11))  # Calo Particles from 1 to 10 inclusive

# Lists to store performance metrics for each CP value
avg_inference_times = []  # mean inference time per event in seconds
std_inference_times = []  # standard deviation of inference time per event
avg_peak_memory = []      # mean peak memory usage in MB
std_peak_memory = []      # standard deviation of peak memory usage

# Loop through each calo particle setting:
for cp in cp_values:
    ...
    for run in range(num_runs):
        ...
        data_test = CCV3(testpath, max_events=500, n_particles=cp)
    ...
    # Compute the mean and standard deviation from the runs.
...
results_df = pd.DataFrame({
    'Calo_Particles': cp_values,
    'InferenceTime_Mean': avg_inference_times,
    'InferenceTime_Std': std_inference_times,
    'PeakMemory_Mean_MB': avg_peak_memory,
    'PeakMemory_Std_MB': std_peak_memory
})
