...
all_cluster_labels = Aggloremative(all_predictions, threshold=0.23)
...
df_CL.to_csv('df_SEC_e_res.csv', index=False)
#df_TICL = calculate_all_event_scores(GT_ind_filt, energies, , TrueEnergy, GT_mult_filt, num_events = len(recon_ind))

#df_TICL.to_csv('df_MT_e_res.csv', index=False)
